...
#Ryan's model framework but using GRU instead for training base models for comparison.
class basicgru(nn.Module):
    def __init__(
        ...
        super().__init__()
        #we will use num_lstm_layers for convenience of parameters.
        self.gru = nn.GRU(
            input_size=num_features,
            hidden_size=hidden_size,
            num_layers=num_lstm_layers,
        ...
        # Project hidden_size * 2 back to num_features for the classification head
        self.proj = nn.Linear(hidden_size, num_features)
        self.layer_norm = nn.LayerNorm(num_features)
